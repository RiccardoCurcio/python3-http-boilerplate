...
class ReadCompaniesAdapter(Adapter):
    ...
    async def adapt(self) -> Company:
        ...
        self.__company_query.ids = query.get("ids", None)  # Union[List[ObjectId], None] = None
        self.__company_query.searchKeys = query.get("searchkeys", None)  # : Union[List[str], None]
        self.__company_query.searchValues = query.get("searchvalues", None)  # : Union[List[str], None]
